...
const webPath = "/";
const filesPath = "./web";
const staticFiles = express.static(path.join(process.cwd(), filesPath), {
...
app.use("/albums", (req, res) => {
    ...
    fs.readFile(path.join(process.cwd(), filesPath, "json/albums.json"), (err, data) => {
        if (err) {
            res.sendStatus(404);
            return;
        }
        try {
            ...
            data = data.filter(e => e.userId === userId);
...
app.use("/photos", (req, res) => {
    res.setHeader("Content-Type", "application/json");
    const albumId = parseInt(req.query.albumId);
    if (isNaN(albumId)) {
        res.send("[]");
        return;
    }
    fs.readFile(path.join(process.cwd(), filesPath, "json/photos.json"), (err, data) => {
        if (err) {
            res.sendStatus(404);
            return;
        }
        try {
            data = JSON.parse(data);
            data = data.filter(e => e.albumId === albumId);
            res.send(JSON.stringify(data));
        } catch (err) {
            res.sendStatus(500);
        }
...
app.use(function (req, res) {
    console.error(`No route to: ${req.originalUrl}​​​​​`);
    res.sendStatus(404);
});
app.listen(port, () => {
    console.log(`To access server open browser and enter following address: http://localhost:${port}​​​​​`)
});